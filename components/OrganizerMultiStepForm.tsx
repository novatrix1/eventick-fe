...
interface OrganizerMultiStepFormProps {
...
const OrganizerMultiStepForm: React.FC<OrganizerMultiStepFormProps> = ({
  formData,
  loading,
  onInputChange,
  onSubmit,
  ...
  const addSocialMediaField = () => {
    const newSocialMedia = [...(formData.socialMedia || [])];
    newSocialMedia.push({ type: 'website', url: '', name: formData.companyName });
    onInputChange('socialMedia', newSocialMedia);
  };

  const removeSocialMediaField = (index: number) => {
    if (formData.socialMedia && formData.socialMedia.length > 1) {
            ...
            {(formData.socialMedia || []).map((item, index) => (
              <View key={index} className="mb-4 p-4 bg-white/5 rounded-xl">
                <View className="flex-row items-center mb-3">
                  ...
                  {(formData.socialMedia && formData.socialMedia.length > 1) && (
                    <TouchableOpacity
...
export default OrganizerMultiStepForm;